...
func TestHTTPHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "bad get request (path)",
			method: http.MethodGet,
			url:    "/api/data/Valid_Rule_4",
			scan:   ScanNeededPath,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (query key)",
			method: http.MethodGet,
			url:    "/api/data?user=bob&Valid_Rule_1=age",
			scan:   ScanNeededQueryKey,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (query value)",
			method: http.MethodGet,
			url:    "/api/data?user=bob&age=Valid_Rule_1",
			scan:   ScanNeededQueryVal,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (headers key)",
			method: http.MethodGet,
			url:    "/api/data",
			headers: map[string]string{
				" Valid_Header_Key_Rule ": "X-Custom-Header",
			},
			scan:   ScanNeededHeadersKey,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (headers value)",
			method: http.MethodGet,
			url:    "/api/data",
			headers: map[string]string{
				"X-Custom-Header": "Valid_Rule_2",
			},
			scan:   ScanNeededHeadersVal,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (cookies key)",
			method: http.MethodGet,
			url:    "/api/data",
			headers: map[string]string{
				"Accept": "application/json",
				"cOoKiE": "session=clean_value; Valid_Rule_3=auth_token",
			},
			scan:   ScanNeededCookiesKey,
			status: http.StatusBadRequest,
		},
		{
			name:   "bad get request (cookies value)",
			method: http.MethodGet,
			url:    "/api/data",
			headers: map[string]string{
				"Accept": "application/json",
		...
		},
		{
			...
			method: http.MethodPost,
			url:    "/api/data",
			body:   []byte(`{"user": "bob", "age": "Valid_Rule_4"}`),
			scan:   ScanNeededBody,
			status: http.StatusBadRequest,
		},
	}
